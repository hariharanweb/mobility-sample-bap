...
describe('Confirm Service', () => {
  it('should test whether getContextWithContext was called', async () => {
    await ConfirmService.confirm(request);
  ...
  it('should test storeConfirmResult', async () => {
    const response = {};
    ConfirmService.storeConfirmResult(response);
    expect(MessageRespository.storeResult).toBeCalled();
  });

  it('should test getConfirmResult', () => {
    const messageId = {};
    ConfirmService.getConfirmResult(messageId);
