...
describe('Look Up Service', () => {
  ...
  it('should test whether cache is checked in getPublicKeyWithUkId', () => {
    LookUpService.getPublicKeyWithUkId(sample);
    expect(Cache.getCache).toBeCalled();
  });

  it('should test whether api call is made and public key is returned in getPublicKeyWithUkId', async () => {
    const publicKey = await LookUpService.getPublicKeyWithUkId(sample);
    expect(Api.doPost).toBeCalled();
    expect(publicKey).toBe('OK');
  });
